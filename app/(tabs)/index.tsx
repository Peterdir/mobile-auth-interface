...
export default function HomeScreen() {
  ...
  useEffect(() => {
    // Toggle tab bar visibility based on chat states
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setActiveFriends(
          friendsData.filter((f) => f.status.type !== "offline"),
        );

        // Fetch Servers
        try {
          const myServers = await serverApi.getMyServers();
          const mappedServers: ServerUI[] = myServers.map(
            (s: ServerResponse) => ({
              id: s.id.toString(),
              name: s.name,
              iconUrl: s.iconUrl,
  ...
  const getFilteredData = () => {
    ...
    const matchingFriends = allFriends.filter(
      (f) =>
  ...
  const handleDeleteServer = (server: ServerUI) => {
    Alert.alert(
      "Xóa Server",
      `Bạn có chắc chắn muốn xóa server "${server.name}"? Hành động này không thể hoàn tác.`,
      [
        { text: "Hủy", style: "cancel" },
        {
          text: "Xóa",
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "Lỗi",
                "Không thể xóa server. Bạn có phải là chủ sở hữu không?",
              );
            }
          },
        },
  ...
  const renderServerItem = (server: ServerUI) => {
    ...
    return (
      <View
        key={server.id}
        className="flex-row items-center mb-2 w-full justify-center relative"
      >
        {isSelected && (
    ...
    return (
      ...
      >
        <View className="relative mr-3">
          <Image
            source={{ uri: avatar }}
            className="w-10 h-10 rounded-full bg-discord-element"
          />
          {status && (
            <View
              className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-[3px] border-discord-background ${getStatusColor(status)}`}
            />
          )}
        </View>
        <View className="flex-1 mt-1">
          <View className="flex-row justify-between items-center">
            <Text
              className={`text-base flex-1 ${item.unreadCount > 0 ? "text-white font-bold" : "text-discord-text-muted font-medium"}`}
              numberOfLines={1}
            >
              {name}
            </Text>
          ...
          <Text
            className={`text-sm mt-0.5 ${item.unreadCount > 0 ? "text-discord-text-normal" : "text-discord-text-muted"}`}
            numberOfLines={1}
          >
  ...
  return (
    <SafeAreaView
      className="flex-1 bg-discord-background flex-row"
      edges={["top"]}
    >
            ...
            <View className="mb-2 relative w-full items-center justify-center">
              ...
              >
                <IconButton
                  icon="message-text"
                  size={24}
                  iconColor={selectedServerId === "dm" ? "white" : "#DBDEE1"}
                  style={{ margin: 0 }}
                />
              </TouchableOpacity>
            </View>
      ...
      {selectedServerId === "dm" ? (
        ...
        <View className="flex-1 flex-row">
          {/* Channel Sidebar - Visible only when NOT in chat */}
          {!isChatFullscreen && (
            <ServerChannelList
